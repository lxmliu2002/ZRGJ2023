...
functions_prename_list, functions_name_list, parameters_list, returns_list, urls_list = [],[],[],[],[]

def write_dict_to_excel(data_dict, filename):
    # 获取.py文件所在目录
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # 生成完整的文件路径
    file_path = os.path.join(current_dir, datetime.now().strftime("%Y-%m-%d %H-%M") + ' ' + filename.replace(':', '_'))
    # 创建一个新的Excel工作簿
    workbook = Workbook()
    # 获取活动的工作表
...
def write_data_to_csv(data, filename):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(current_dir, datetime.now().strftime("%Y-%m-%d %H-%M") + ' ' + filename)
...
def spider_big(url):
    ...
    if status_code == 200:
        ...
        for div in divs_all:
            ...
            function_link = "https://numpy.org/devdocs/reference/" + str(function_link_element)

            if function_link == 'https://numpy.org/devdocs/reference/routines.polynomials.classes.html':
                continue
            else:
                if url == 'https://numpy.org/devdocs/reference/routines.polynomials.html':
...
def spider_small(function_link):
    ...
    if status_code == 200:
        ...
        function_prename = selected_elements1[0].get_text() if selected_elements1 else 'None'

        selected_elements2 = divs_all[0].select('span.sig-name.descname')
        function_name = selected_elements2[0].get_text() if selected_elements2 else 'None'
        ...
        return_ = soup_document.select("dd.field-even dl.simple dt span.classifier")[0].get_text() if soup_document.select("dd.field-even dl.simple dt span.classifier") else 'None'
        ...
        for div in divs_all:
            if any(div):
                ...
                for div_ in div_s_all:
                    function_link_element = div_.find(class_='reference internal').get('href')
                    if function_link_element.startswith('generated/'):
                        function_link_element = function_link_element[len('generated/'):]
                    function_link = "https://numpy.org/devdocs/reference/generated/" + str(function_link_element)
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = list(tqdm(executor.map(spider_big, urls), total=len(urls)))
    ...
    write_data_to_csv(data, 'numpy说明文档(多线程).csv')

    print("爬取完毕！")
    # 调用方法生成excel
    write_dict_to_excel(data, 'numpy说明文档(多线程).xlsx')